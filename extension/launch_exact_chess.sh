...
EXT_DIR="/mnt/us/extensions/exact-chess"
APP_BIN="$EXT_DIR/bin/armhf/exact-chess"
APP_LOG="/mnt/us/exact-chess.log"
APP_TITLE="Exact Chess"
APP_LOADER="$EXT_DIR/lib/armhf/ld-linux-armhf.so.3"
RUNTIME_MODE="${EXACT_CHESS_RUNTIME:-auto}"
...
try_launch() {
    ...
    DISPLAY=:0 GDK_NATIVE_WINDOWS=1 EXACT_CHESS_ENGINE="$ENGINE_PATH" "$@" >>"$APP_LOG" 2>&1 &
    pid="$!"
    sleep 1
