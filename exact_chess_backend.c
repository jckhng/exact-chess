...
void exact_chess_backend_reset(ExactChessBackend *backend) {
    ...
    exact_chess_backend_init(backend);
}
...
ExactChessGameState exact_chess_backend_state(ExactChessBackend *backend) {
    ...
    if (result == CHESS_RESULT_IN_PROGRESS) {
        return state->check_state == CHECK_STATE_CHECK ? EXACT_CHESS_CHECK : EXACT_CHESS_ONGOING;
    }
    if (rule == CHESS_RULE_CHECKMATE) {
        return EXACT_CHESS_CHECKMATE;
    }
    if (rule == CHESS_RULE_STALEMATE) {
        return EXACT_CHESS_STALEMATE;
    }
    return EXACT_CHESS_DRAW;
}
